...
model = Model()
model.load_state_dict(torch.load('cyclonet\saved_modelcyclone.pth', map_location=torch.device('cpu')))
model.eval()
...
def index():
    title = "StormSense"
    return render_template("index.html", rain=1, x=0, title=title)
...
def predict():
    ...
    print(img_date, img_time, latitude, longitude)
    ...
    if imagefile and allowed_img(imagefile.filename):
        ...
        # Open image using PIL and preprocess
        ...
        if predicted_intensity >= 0 and predicted_intensity <= 30:
            r = 50
        elif predicted_intensity > 30 and predicted_intensity <= 50:
            r = 70
        elif predicted_intensity > 50 and predicted_intensity <= 70:
            r = 80
        elif predicted_intensity > 70 and predicted_intensity <= 100:
            r = 90
...
#         image_time = request.form.get('image_time')
#         latitude = request.form.get('latitude')
#         longitude = request.form.get('longitude')
...
#             prediction = "Placeholder Prediction"  # Replace with your calculation logic
...
#         )
#     return redirect(url_for('cyclonet\templates\index.html'))
...
def display_image(filename):
    ...
    return render_template('cyclonet\templates\image.html', filename=filename)
